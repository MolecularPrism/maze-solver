...
maze_cell** parse_maze( FILE* maze_file, int dimension )
{
  ...
  int         row = 0;
	int         column = 0;
	maze_cell** maze = NULL; //pointer to row with the double **
...
void generate_all_paths(char*** pathsetref, int* numpathsref, maze_cell** maze, int dimension, int row, int column, char* path)
{
	...
	if ( column < 0 || row < 0 || row >= dimension || column >= dimension || maze[row][column].character == '*' || maze[row][column].visited == 'Y' ) {
    return;
  }
  ...
  /* Otherwise deals with the recursive case.  Pushes the current coordinate onto the path
	  ...
	  path_length = strlen( path ); //get length of char* path
	  new_path = ( char * ) calloc( path_length + 2, sizeof( char ) ); //allocate space for char* new_path which is 2 spaces larger than char* path
	  new_point = ( char * ) calloc( 2, sizeof( char ) ); //allocate space for a new cell that you've moved onto
	  new_point[0] = maze[row][column].character; //assign character of current cell to new_point[0]
	  if ( path_length ) {
		  new_path = strcat( new_path, path ); //you are just copying path to a bigger space new_path
	  }
	  new_path = strcat( new_path, new_point ); //then you are just concatenating the current cell to new_path
    free(new_point); //reset new_point (current cell) data

    if ( column == ( dimension - 1 ) ) {             /*IF YOU ARE ON THE RIGHTMOST COLUMN...*/
		...
		//*pathsetref is a 2D solution paths string array
		//when you reach finish, you wanna just add space for the new path solution
	    *pathsetref = ( char** ) realloc ( *pathsetref, ( (*numpathsref) + 1 ) * sizeof( char* ) );

		//you are just adding enough space in the column for the path 
      (*pathsetref)[*numpathsref] = ( char* ) calloc( strlen( new_path ) + 1, sizeof( char ));

		//you are copying the new_path solution to the array
	    strcpy( (*pathsetref)[*numpathsref], new_path ); //copy new_path to row


	    (*numpathsref)++; //next row of *pathsetref 2d array
      return;
    } else {
...
{
	int short_index = 0;
	int shortest_dist = strlen(*pathset);
	
	for (int i = 0; i < numpaths ; i++) {
		if (strlen(*( pathset+ i)) < shortest_dist) {
			
			shortest_dist = strlen(*(pathset + i));
			short_index = i;
		}
		
	}
	strcat(outputbuffer, "Shortest path: ");
	strcat(outputbuffer, *(pathset + short_index));
...
void construct_cheapest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	...
	int cheapest_cost = 100000000;
	char cost_buffer[10000];
	int sum = 0;
	

	for (int i = 0; i < numpaths; i++) {
		sum = 0;
		for (int j = 0; j < strlen(*(pathset + i)); j++) {
			sum = sum + (int) ( * (*(pathset + i) + j) - '0');
		}
		if (cheapest_cost >= sum) {
			cheapest_cost = sum;
			cheapest_index = i;
		}
	}

	sprintf(cost_buffer, "%d", cheapest_cost);
